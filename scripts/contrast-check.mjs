...
for(const u of urls){const p=await b.newPage();await p.setViewport({width:1280,height:800});try{await p.goto(u,{waitUntil:'networkidle2',timeout:60000});}catch(_){}; const r=await p.evaluate(SCAN); console.log(u, JSON.stringify(r)); if(r.low>0)fail++; await p.close();}
await b.close();
