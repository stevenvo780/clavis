...
![Grafo comparativo 2D: polis griega y ciudad romana](../03_laminas_generadas/comparacion_ciudades_griegas_romanas.png)

---

## 1. Introducción

La ciudad antigua no es un fenómeno homogéneo. Aunque Grecia y Roma comparten raíces mediterráneas, sus modelos urbanos responden a **lógicas políticas, sociales y técnicas distintas**. Esta nota se concentra en la morfología, los equipamientos y la implantación territorial. La [comparación ontológica](./comparacion_ontologica_ciudades_griega_y_romana.md) desarrolla por separado los conceptos de `polis`, `urbs`, `civitas`, ciudadanía y escala.
...
[`generar_grafo.py`](../../../04_Recursos_Tecnicos/scripts/generar_grafo.py)
se organiza así:
...
desarrollan en [`Urbs`, `civitas`, `urbanitas`, `cité` y `ville`](./urbs_civitas_urbanitas_glosario_romano.md).
La organización griega de acrópolis, ágora y `stoa` se explica en
[Acrópolis y Ágora de Atenas](./acropolis_y_agora_de_atenas_distribucion_espacial.md);
los ejes y la retícula, en [Hipódamo de Mileto](./hipodamo_de_mileto_planeacion_clasica.md)
y [Vías griegas y romanas](./vias_griegas_y_romanas_funciones_y_escala.md).
...
*Generado con
[`generar_grafo.py`](../../../04_Recursos_Tecnicos/scripts/generar_grafo.py),
`matplotlib` y `networkx`, a 300 dpi.*
